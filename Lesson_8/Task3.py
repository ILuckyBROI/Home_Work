...
def type_logger(Fun):
    @wraps(Fun)
    def reapper(*args, **kwargs):
        number = [el for el in (*args, *kwargs.values())]
        n = [f'{Fun.__name__}({el}: {type(el)})' for el in number]
...
@type_logger
def calc_cube(*x, **y):
    number = [el for el in (*x, *y.values()) if isinstance(el, int) or isinstance(el, float)]
